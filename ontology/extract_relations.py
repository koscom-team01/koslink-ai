        ...
        "relations": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "description": {"type": "string", "description": "관계를 설명하는 한국어 문장 (100자 내외)"},
                    "source_quote": {
                        "type": "string",
...
PROMPT_TEMPLATE = """아래는 반도체 기업 "{company_name}"의 사업보고서 '사업의 개요' 본문입니다.
이 본문에서 {company_name}와(과) 다른 회사 사이의 관계(지분투자/계열사/기술 라이선싱/경쟁사/인수합병 등)를
찾아 JSON으로 추출해 주세요.
...
- source_quote는 반드시 아래 본문에서 그대로 발췌한 문장이어야 하며, 요약하거나 새로 쓰면 안 됩니다.
- 본문에 명확한 관계 언급이 없으면 relations를 빈 배열로 반환하세요. 추측하지 마세요.
...
def build_llm_candidates(client: anthropic.Anthropic, model: str, company: dict, usage_totals: dict) -> list[dict]:
...
def main() -> None:
    ...
    if settings.ANTHROPIC_API_KEY:
        ...
        print("⚠️  ANTHROPIC_API_KEY가 없어 LLM 추출(비구조화 경로)은 건너뜁니다 — 구조화 경로만 실행합니다.")
        ...
        if client:
            try:
                all_candidates.extend(
                    build_llm_candidates(client, settings.ANTHROPIC_MODEL, company, usage_totals)
                )
    ...
    if client:
        ...
        print(
            f"   Claude 사용량: 입력 {usage_totals['input_tokens']:,} + "
